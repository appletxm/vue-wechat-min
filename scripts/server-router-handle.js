...
function getMockFile(reqPath, res) {
  try {
    ...
    if (!fs.existsSync(reqPath)) {
      res.status(404).sendFile('assets/images/404_pic.png')
      return false
    }
...
function getVideoFile(compiler, filename, res, next) {
  var newFs = compiler ? compiler.outputFileSystem : fs

  console.info('[get video path]', filename)

  newFs.readFile(filename, function (err, result) {
    if (err) {
      res.send(err)
    } else {
      res.set('content-type', 'video/' + filename.match(/\.(\d)$/))
      res.send(result)
    }
    res.end()
...
function getCfgJsFile(req, res) {
  res.set('content-type', 'application/x-javascript')
  res.send(syncCfg.getText())
  res.end()
}
...
function routerVideoPath(req, res, compiler) {
  // var reqImgPath = req.baseUrl.match(/.+(assets.+)$/)[1]
  var filename = compiler ? path.join(compiler.outputPath, req.baseUrl.match(/.+(assets.+)$/)[1]) : path.join(__dirname, '../' + req.baseUrl + req.path)
  getVideoFile(compiler, filename, res)
}
...
function routerJsFile(req, res, compiler) {
  var filename = path.join(compiler.outputPath, req.baseUrl.replace('/', ''))
  if (req.originalUrl.indexOf('cfg.js') >= 0) {
    getCfgJsFile(req, res)
  } else {
